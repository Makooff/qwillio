...
- **Preuve** : L'accueil est bien pré-synthétisé et servi comme URL à Vapi : `backend/src/services/voice/greeting-audio.service.ts` (3 variantes par client, table `GreetingAudio`, route `/api/voice/greeting/:clientId/:variant`), branché en `firstMessage` (`realtime-orchestrator.service.ts:200-214`). Loin du critère : 3 phrases et non 10, en MP3 et non en PCMA 8 kHz, servies par une requête HTTP de Vapi vers notre API (donc pas un TTFA de 0 ms), désactivées dès que le TTS n'est pas ElevenLabs (`greeting-audio.service.ts:57`) et en parole-à-parole (`realtime-orchestrator.service.ts:206`). Les accusés de réception et phrases d'attente ne sont PAS pré-générés : ce sont des textes synthétisés en direct (`voice-tools.ts:31-123`).
  **Relevé de production (27/08) — la fonctionnalité est ÉTEINTE** : `VOICE_TTS_PROVIDER=cartesia`, et `generate()` refuse de synthétiser dès que le fournisseur n'est pas `11labs` (`greeting-audio.service.ts:57`). Plus aucun accueil n'est pré-enregistré, pour personne. Pire, la garde est posée à la GÉNÉRATION et pas à la LECTURE : `resolveFirstMessage` sert toujours les lignes déjà en base sans vérifier le fournisseur (`realtime-orchestrator.service.ts:213`), donc un client dont les accueils datent de l'époque ElevenLabs s'entend accueillir par une voix et répondre par une autre — exactement le défaut que le commentaire de `greeting-audio.service.ts:51-56` dit vouloir éviter. Portée exacte : `generate()` n'a qu'un seul appelant, `syncVapiAssistant`, qui commence par `invalidate()` (effacement de toutes les lignes) avant de régénérer (`onboarding.service.ts:800` et `:817`). Sont donc concernés les seuls clients **non resynchronisés depuis la bascule vers Cartesia** ; le premier changement de réglage les répare en effaçant leurs lignes, et les rend au passage à la synthèse en direct.
