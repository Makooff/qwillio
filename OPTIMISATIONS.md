...
- **Ce qui manque pour cette ligne précise** : les noms de RUES, et leur chargement EN COURS d'appel. La liste est figée à la construction de l'assistant, donc elle ne peut pas suivre la commune une fois celle-ci identifiée. Il n'existe toujours aucune intégration BeSt Address ni aucun champ adresse dans les outils (`voice-tools.ts`), et la mise à jour de mots-clés en cours de flux demanderait Deepgram Flux — un changement de fournisseur, donc règle 7.
