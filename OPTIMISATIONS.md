...
- **Preuve** : L'accueil est pré-synthétisé et servi comme URL à Vapi : `backend/src/services/voice/greeting-audio.service.ts` (3 variantes par client, table `GreetingAudio`, route `/api/voice/greeting/:clientId/:variant`), branché en `firstMessage` (`realtime-orchestrator.service.ts:200-214`). Loin du critère : 3 phrases et non 10, en MP3 et non en PCMA 8 kHz, servies par une requête HTTP de Vapi vers notre API (donc pas un TTFA de 0 ms), et désactivées en parole-à-parole (`realtime-orchestrator.service.ts:206`). Les accusés de réception et phrases d'attente ne sont PAS pré-générés : ce sont des textes synthétisés en direct (`voice-tools.ts:31-123`).
  ...
  **Corrigé le 10/09, l'arbitrage ayant été tranché** : la variante est FIGÉE à 0 et servie en audio. Tous les appelants entendent donc exactement la même phrase, en échange des centaines de millisecondes de synthèse sur le mot qui décide de l'impression. C'est le prix, il est assumé.

  **Le piège de l'ordre, qui aurait été pire que le défaut** : `available()` rend une URL vers une ligne de base, et la synchronisation EFFACE ces lignes avant de les refaire en tâche de fond. Épingler l'URL sans précaution ouvrait une fenêtre où chaque appel commence sur un 404, c'est-à-dire sur du SILENCE — l'appelant y perdrait bien plus que la latence gagnée. La régénération est donc attendue avant qu'on lise l'URL, l'effacement d'après l'envoi est retiré, et le texte reste le repli quand la synthèse échoue. Quatre des cinq tests de `__tests__/assistant-greeting-source.test.ts` ne vérifient que cet ordre.
...
- **Preuve** : Les percentiles existent, par étage : `voice-metrics.service.ts` rend p50/p95/p99 pour `stt`/`llm`/`tts`/`ttfa`/`total`, exposés sur `/api/webhooks/vapi/health` (`backend/src/controllers/voice-webhook.controller.ts:257`), et une alerte p95 hebdomadaire nomme l'étage fautif (`receptionist-learning.service.ts:152-186`).
  ...
  **L'écran existe depuis le 10/09**, et l'alerte dit enfin quelque chose. Les percentiles étaient calculés, publiés, et lus par personne : `/api/webhooks/vapi/health` n'est ouvert par aucun humain. `voiceLatency` passe donc sur `/api/admin/system`, derrière `adminMiddleware`, et `VoiceLatencyPanel` le rend dans l'onglet Système, la donnée arrivant par `props` puisque l'onglet appelait déjà cette route — une seconde requête ferait diverger les deux moitiés de l'écran au premier rechargement. Le p95 est la colonne mise en avant : un appel fait trente à soixante tours, donc il touche plusieurs fois la queue, et une médiane flatteuse cohabite très bien avec un appel sur cinq qui traîne.

  **L'alerte hebdomadaire nommait un CODE à côté d'un identifiant de client** (`slow_turns`, puis un UUID), c'est-à-dire qu'elle disait d'aller chercher sans dire quoi, ni où, ni si ça valait le déplacement. Elle porte maintenant le nom commercial, le détail chiffré et l'action. Une alerte qu'on doit instruire avant de savoir si elle compte est une alerte qu'on finit par ne plus ouvrir, et c'est le mode d'échec de tous les avertissements de ce dépôt.

  Ce qui manque : la mesure elle-même n'a encore rien à montrer, aucun appel entrant réel n'ayant eu lieu — l'encart le dit plutôt que d'afficher un tableau de tirets, qui se lit comme une mesure ratée.
...
- **Statut** : `PARTIEL`
- **Preuve** : Les trois moyens sont là : outils exécutés EN PARALLÈLE (`Promise.all`, `backend/src/services/voice/realtime-orchestrator.service.ts:386`), meublage non bloquant à l'invocation + relance à 1 200 ms (`voice-tools.ts:133-153`, `env.ts:360`), et préchargement spéculatif de l'agenda dès que l'appelant nomme un jour (`availability-speculator.ts`, déclenché en `realtime-orchestrator.service.ts:302`). Ce qui manque : le meublage est du TEXTE synthétisé, pas de l'audio pré-généré ; le préchargement se fait sur la date entendue, pas sur l'identification du numéro appelant.
- **Le plafond de 500 ms est MESURÉ depuis le 10/09** : les trois moyens de le couvrir existaient et rien ne vérifiait qu'ils suffisaient. Le relevé lit le CONTRAT DE MEUBLAGE, pas le chronomètre seul — un outil lent derrière une phrase de meublage ne laisse aucun silence, un outil rapide sans meublage en laisse toute sa durée, donc la durée seule ne décrit pas ce que l'appelant entend. Publié à chaque analyse même quand il vaut zéro, et l'action nomme les outils à équiper. Tests : `__tests__/tool-silence.test.ts`, 5 cas, dont l'outil en erreur qui est écarté — il a déjà son propre relevé, et le compter ici ferait remonter deux fois le même incident sous deux noms.
...
- **La table slot → seuil existe désormais** : `buildCustomEndpointingRules` (`backend/src/services/voice/speech-plans.ts`), posée dans `startSpeakingPlan.customEndpointingRules`. Vapi fait matcher une règle de type `assistant` sur la DERNIÈRE PHRASE DE L'AGENT, ce qui est le bon accrochage : on ne sait pas ce que l'appelant va dire, on sait toujours ce qu'on vient de lui demander. Deux paliers par langue (fr/en/nl) : 2,5 s quand l'agent demande une adresse, une rue ou un code postal ; 3 s quand il demande un e-mail ou fait épeler. Vapi documente ce champ pour ce cas précis, « data collection scenarios, such as gathering phone numbers or addresses, or for spelling tasks ». Tests : `__tests__/speech-plans.test.ts` (8 cas, dont un tour ordinaire qui ne doit PAS être rallongé) et `__tests__/vapi-schema.test.ts` (les clés envoyées).
- **Ce qui manque pour `DÉJÀ FAIT`** : la mesure. Le critère demande de vérifier sur un appel qu'une adresse dictée en trois blocs n'est plus coupée, et aucun appel entrant réel n'a été passé. `regexOptions` n'est pas envoyé non plus, délibérément : la référence d'API en donne la forme, l'exemple de la documentation l'omet, et se tromper ferait refuser l'assistant entier — l'insensibilité à la casse est donc écrite dans les motifs.
...
- **Statut** : `PARTIEL`
- **Preuve** : Le paramètre est à 2 : `numWords: tuning.bargeInWords` (`backend/src/services/voice/speech-plans.ts:274`), `VOICE_BARGE_IN_WORDS` défaut 2 (`env.ts:169`), test qui interdit le retour à 0 (`__tests__/speech-plans.test.ts:27-36`). Deux manques : aucun test avec un fichier de bruit domestique, et surtout le mode parole-à-parole force `numWords: 0` (faute de transcripteur) — sur ce chemin le critère est faux par construction, seul `voiceSeconds` protège.
- **Le curseur de ce chemin-là est désormais SÉPARÉ (10/09)** : les deux chemins n'ont pas les mêmes défenses et partageaient pourtant un réglage unique, ce qui rendait l'écart invisible. En classique le bruit est trié deux fois, sur l'énergie puis sur les mots transcrits ; en parole-à-parole la première passe est la seule. `VOICE_REALTIME_BARGE_IN_VOICE_SECONDS` permet donc de protéger ce chemin sans ralentir l'interruption volontaire sur l'autre. **Le défaut ne bouge pas**, et c'est délibéré : monter ce seuil change la charge envoyée à Vapi, et un champ hors bornes fait refuser l'assistant ENTIER, donc toute la flotte (6octies). Il se monte après un `npm run voice:validate` qui confirme que l'API l'accepte, jamais sur une intuition.
...
- **Statut** : `PARTIEL`
- **Le code mort est branché** : `recoveryLine()` et `newRepairState()` étaient écrits ET testés mais appelés depuis AUCUN point du runtime. `recordBargeIn` pose maintenant `pendingHardBargeIn` sur la session, `callSessionStore.takeRecoveryLine` le consomme, et `llmStreamService.withRecovery` ajoute au tour suivant une consigne système DE QUEUE : ouvrir par la phrase, puis répondre, sans reprendre la phrase interrompue. En queue comme l'humeur, pour que le long préfixe reste cachable. La retenue reste dans le module (au plus deux par appel, jamais deux d'affilée, jamais sur un acquiescement coupé court). Tests : `__tests__/conversational-repair.test.ts` (6 cas de branchement), `__tests__/llm-stream.test.ts` (3 cas sur ce que le modèle reçoit vraiment).
- **Ce qui reste hors de portée, et pourquoi** : la reprise après un FAUX déclenchement, c'est-à-dire du bruit suivi d'aucune parole. Vapi n'expose aucun champ de reprise — son `stopSpeakingPlan` ne porte que `numWords`, `voiceSeconds`, `backoffSeconds` et les deux listes de phrases, vérifié sur la référence d'API — et `resume_false_interruption` / `false_interruption_timeout` sont des paramètres LiveKit, pas Vapi. Sans parole de l'appelant il n'y a pas non plus de tour de modèle où se raccrocher : c'est le relais d'inactivité (`buildIdleMessagePlan`, 5 s) qui couvre ce cas, moins bien.
...
- **Statut** : `PARTIEL`
- **Preuve** : `spokenPrefix` (`backend/src/services/voice/spoken-prefix.ts`) ramène le dernier énoncé de l'agent à sa part jouée, et `llmStreamService.withHeardOnly` réécrit ce message dans l'historique avant le tour suivant. La durée vient de la seule mesure que la chaîne fournisse : `recordBargeIn` relève depuis combien de temps l'agent parlait (`assistantSpeakingSince`, posé par `speech-update`), et la range dans `interruptedSpeechMs`, consommée à la lecture. Test : `__tests__/spoken-prefix.test.ts`, 10 cas, dont celui du critère — six secondes d'énoncé coupées à une seconde et demie, et le créneau du milieu ne survit pas dans l'historique.
- **Ce qui reste une ESTIMATION, et c'est le mot juste** : la synthèse appartient à Vapi, qui ne rend aucun horodatage au mot. La durée se convertit en caractères par un débit de parole (15 c/s en français), donc le module dit ce que l'appelant a *plausiblement* entendu, pas ce qu'il a entendu. L'arbitrage est assumé dans un sens : garder du texte jamais joué ramène le défaut d'origine, en retirer un peu trop fait au pire répéter l'agent. Deux garde-fous : rien n'est touché au-delà de 85 % de l'énoncé, où l'écart n'est plus que du bruit de mesure, et la coupe se fait sur un mot entier — un mot tranché en deux se lit comme une faute de frappe et le modèle le réécrirait.
- **Ce que ça ne couvre pas** : le chemin CLASSIQUE, où le modèle est tenu par Vapi. On ne possède pas cet historique, aucun champ ne l'expose, et rien ne dit si Vapi tronque de son côté.
...
- **Statut** : `PARTIEL`
- **Preuve** : `false-cut.ts` relève le sens qui manquait — l'agent qui coupe l'APPELANT. `recordBargeIn` incrémente `falseCuts`, l'appel le porte jusqu'au rapport de fin, et `receptionist-learning.service.ts` publie le taux à chaque analyse, avec la cible de 5 % et l'action qui va avec (monter `VOICE_ENDPOINTING_MS`). Publié même sous la cible : sans dénominateur affiché, un taux qui remonte ne se voit pas remonter. Tests : `__tests__/false-cut.test.ts`, 13 cas.
- **Deux conditions, jamais une seule** : l'appelant reprend la parole dans les 1 200 ms qui suivent le début de l'énoncé, ET son tour précédent s'arrêtait au milieu d'une pensée. Une seule ne trie rien — un « non non » lâché aussitôt est une réaction, pas une coupure, et une phrase en suspens peut simplement rester en suspens. La fin de pensée se lit sur la ponctuation forte quand le transcripteur en met, sinon sur le dernier mot, avec une liste de liaison PAR LANGUE (« een » est un déterminant néerlandais et rien du tout en français).
- **Ce qui reste hors de portée** : le découpage lui-même appartient au transcripteur, qui ne dit jamais qu'il s'est trompé. Ce compteur lit une SIGNATURE, pas un aveu, et sa justesse ne se vérifiera que sur des appels réels — dont aucun n'a encore eu lieu. Le seuil de confiance du transcripteur n'est par ailleurs pas exposé par Vapi : ce qui se règle sur ce taux est l'endpointing, pas une confiance.
...
- **Statut** : `ABSENT`
- **Le mécanisme existe désormais, la source de données non** : `buildVocabularyField` (`backend/src/services/voice/speech-plans.ts`) souffle une liste de termes au transcripteur, et `buildAssistantForCall` y verse le nom de l'entreprise, celui de l'agent et les intitulés de prestations. Fenêtre plafonnée à 60 termes, dédoublonnée, mots vides écartés, casse conservée (Deepgram s'en sert). Tests : `__tests__/speech-plans.test.ts`, 7 cas.
- **Le piège, qui aurait coupé la flotte** : Vapi n'expose pas le même champ selon le modèle Deepgram. `keyterm` accepte des expressions et n'existe que sur Nova-3 ; `keywords` ne prend que des mots seuls et couvre Nova-2 et en dessous. Or nos langues ne tournent pas sur le même modèle — français et anglais en Nova-3, néerlandais en Nova-2 — et envoyer le mauvais champ ferait refuser l'assistant ENTIER. Le champ est donc choisi par modèle, et les expressions sont découpées en mots sur Nova-2.
- **Il ATTEINT enfin un appel, depuis le 10/09** : le mécanisme était construit, testé, et n'était passé qu'à `buildAssistantForCall` — c'est-à-dire à l'assistant qui ne décroche JAMAIS, le numéro entrant épinglant l'assistant enregistré (6quindecies). Le vocabulaire partait donc dans le vide. Les deux écritures de l'assistant enregistré (`onboarding.service.ts`, création et synchronisation) le portent maintenant, lu sur le profil comme à l'appel.
- **Un second défaut trouvé au même endroit, et plus grave** : la synchronisation décidait la langue avec `isFrenchClient(client) ? 'fr' : 'en'`, sans le néerlandais que la création connaissait. Un client flamand naissait donc correct et repassait en ANGLAIS — transcripteur ET voix — à la première sauvegarde de n'importe quel réglage. Rien ne le signalait : le PUT réussit, l'écran dit enregistré, et le prochain appelant flamand est transcrit en anglais. Les deux chemins lisent désormais la langue du PROFIL, la même source que l'appel. Tests : `__tests__/assistant-speech-sync.test.ts`, 4 cas, dont un qui interdit tout `buildRealtimePlans` sans options dans ce fichier — un troisième chemin d'écriture ajouté demain retomberait sinon dans le même trou en silence.
- **Ce qui manque pour cette ligne précise** : les noms de RUES, et leur chargement EN COURS d'appel. La liste est figée à la construction de l'assistant, donc elle ne peut pas suivre la commune une fois celle-ci identifiée. Le champ `address` existe désormais dans les outils (`voice-tools.ts`) et son contenu est normalisé à l'écriture (BEL-6), mais il ne sert qu'APRÈS coup : il ne souffle rien au transcripteur pendant que l'appelant dicte. Il n'existe aucune intégration BeSt Address, et la mise à jour de mots-clés en cours de flux demanderait Deepgram Flux — un changement de fournisseur, donc règle 7.
...
- **Statut** : `DÉJÀ FAIT`
- **Preuve** : `backend/src/utils/be-communes.ts`, 45 paires dont les deux noms diffèrent, et `normaliseAddress` qui réécrit la commune dictée dans la langue du CLIENT — c'est lui qui relit la fiche, pas l'appelant. Les dix-neuf bruxelloises sont couvertes : treize portent deux noms, les six autres (Anderlecht, Etterbeek, Evere, Ganshoren, Jette, Koekelberg) s'écrivent pareil et n'ont rien à traduire. La périphérie suit avec Crainhem et Rhode-Saint-Genèse, seules communes à facilités dont le nom change. Branché sur la capture : `captureLead` normalise l'adresse à l'écriture (`tool-runtime.service.ts`), et le champ `address` existe dans `voice-tools.ts`. Tests : `__tests__/be-communes.test.ts`, 13 cas dans les deux langues.
- **Les deux pièges, tous deux réintroduits une fois pour vérifier que le test tombe** : la commune ne se remplace que si elle suit un code postal ou termine la ligne, sinon « chaussée de Wavre, 1160 Oudergem » voyait sa RUE traduite ; et une entrée dont les deux noms sont identiques ne change aucune sortie, donc un test interdit d'en ajouter — une s'était glissée dans la périphérie.
...
- **Statut** : `ABSENT`
- **Le biasing existe maintenant, `sounds_like` non** : `buildVocabularyField` (`backend/src/services/voice/speech-plans.ts`) souffle au transcripteur les mots propres au client — nom de l'entreprise, nom de l'agent, intitulés de prestations — via `keyterm` (Nova-3) ou `keywords` (Nova-2), le champ étant choisi par modèle. C'est le mécanisme que cette ligne réclame, appliqué au vocabulaire qu'on possède déjà.
- **Ce qui manque** : `sounds_like` lui-même, qui n'existe pas chez Deepgram — c'est une fonction de prononciation d'autres fournisseurs. Et la liste de patronymes flamands : elle n'est nulle part, et la constituer proprement (Vandenberghe, Vermeulen, De Smet, Janssens…) puis vérifier qu'elle améliore réellement la transcription demande des appels réels, pas une intuition.
...
- **Statut** : `PARTIEL`
- **L'intention est couverte, la lettre est hors de portée.** Ce que cette ligne veut, c'est que l'appelant entende quelque chose quand le TTS meurt. C'est en place, mais par un autre chemin que celui décrit : `voice.fallbackPlan` (`backend/src/services/voice/speech-plans.ts`) liste deux voix ElevenLabs de secours quand Cartesia ne répond pas, avec de vrais identifiants par défaut (`VAPI_VOICE_FALLBACK_1` / `_2`, Elli et Bella) — un secours vide serait pire que pas de secours, puisqu'il masquerait la panne. Le repli est armé sur tous les appels, et il bascule de TTS, pas vers un enregistrement.
- **La première phrase, elle, survit déjà à un TTS mort**, et par accident d'une autre optimisation : l'accueil pré-rendu est servi en OCTETS via `firstMessage` (`greeting-audio.service.ts`). Un moteur de synthèse en panne à l'ouverture de l'appel produit quand même une phrase intelligible.
- **Ce qui n'est pas atteignable depuis ce dépôt** : la banque de cinq à dix phrases servies EN COURS d'appel. Vapi n'accepte de l'audio pré-enregistré qu'en `firstMessage`, et le flux média lui appartient : rien ici ne peut y injecter du son une fois l'appel commencé. Même famille que LAT-4 et REL-9. Le chien de garde sur le temps de premier son tombe pour la même raison — on ne mesure pas un son qu'on ne transporte pas.
...
- **Ce qui manque pour `DÉJÀ FAIT`** : la preuve. Le critère demande de couper le TTS en plein appel, ce qui suppose un appel réel et une panne provoquée chez le fournisseur. Et rien ne nous dit aujourd'hui qu'un repli a servi : Vapi n'émet aucun événement pour ses bascules internes, ce que TST-8 écrit déjà noir sur blanc. Le taux publié ne compte que les replis dont NOUS sommes témoins.
...
- **Statut** : `PARTIEL`
- **Preuve** : L'échec est CONSTATÉ et transformé en rappel : `logTransfer` écrit un `CallTransfer` avec `transferStatus`, `failedReason`, `callbackRequested` et une priorité haute (`backend/src/services/client-call.service.ts:359-401`), alimenté par l'événement `transfer-update` (`voice-webhook.controller.ts:115-123`). **La durée de sonnerie est désormais bornée** : `dialTimeout: env.VOICE_TRANSFER_RING_SECONDS` (20 s, contre 60 par défaut chez Vapi), posé aux DEUX endroits qui construisent un plan de transfert — `warm-transfer.service.ts` pour le plan par appel et `voice-tools.ts` pour le repli statique, un repli qui sonnerait trois fois plus longtemps que le chemin normal étant un piège. Tests : `__tests__/warm-transfer.test.ts`, 3 cas.
- **Ce qui manque encore** : la reprise en main proprement dite. Vapi expose bien `transferPlan.fallbackPlan` (`{ endCallEnabled, message }`, « destination unreachable, busy, or not human »), mais la référence d'API le réserve à `warm-transfer-experimental` et aux transferts aveugles dont l'organisation a activé la détection d'issue. Sur notre mode (`warm-transfer-say-summary`) il serait accepté par le schéma et ignoré à l'exécution : l'envoyer afficherait une reprise qu'on n'a pas, et un test l'interdit explicitement. **Changer de mode pour l'obtenir est un arbitrage produit** — poser un mode expérimental sur le chemin d'une urgence — donc règle 7 : demander, ne pas trancher. Rien n'est non plus testé sur non-réponse / occupé / répondeur, ce qui demande des appels réels.
...
- **L'écran existe (10/09)** : `GET /my-dashboard/transfers` et `TransferFunnel`, sur la page Appels. Les compteurs vivaient à l'échelle de la FLOTTE, sur la route de santé — utile à qui exploite la plateforme, muet pour le gérant qui demande « pourquoi l'agent ne m'a pas passé l'appel ». L'écran répond à cette question-là : tentés, aboutis, échoués, et les causes les plus fréquentes d'abord. Tests : `__tests__/transfer-report.test.ts`, 7 cas.
- **Deux refus assumés dans cet écran** : il n'affiche PAS d'étape « ça a sonné ». La ligne de transfert ne porte que trois états et Vapi ne nous informe d'aucune sonnerie ; inventer un palier intermédiaire rendrait le reste suspect, et la cause le remplace avantageusement (« le poste sonnait occupé » dit qu'il a sonné, et dit en plus quoi faire). Et un échec SANS cause reste compté, sous un libellé qui le dit : le taire ferait un total de causes inférieur au nombre d'échecs, c'est-à-dire l'écart qu'un lecteur remarque sans pouvoir l'expliquer. Une tentative restée ouverte est comptée à part plutôt que rangée d'un côté — un transfert supposé réussi qui ne l'était pas est exactement ce qui a fait croire que l'agent transférait.
- **Ce qui manque pour `DÉJÀ FAIT`** : le vrai code lu sur le fil, qui demande de le récupérer côté opérateur (Twilio), comme l'action le dit elle-même.
...
- **Statut** : `PARTIEL`
- **L'étape existe, et elle est en PREMIER** : `ClientSetupForwarding.tsx` ouvre par « Coupez la messagerie de votre opérateur », avec `##002#` composable en un geste, au-dessus du code de renvoi. Elle ne s'affiche que sur les renvois CONDITIONNELS (`voicemailRisk`, `lib/forwarding-codes.ts`) : le renvoi total prend l'appel avant qu'aucune condition ne s'évalue, et l'afficher là ajouterait un geste inutile à l'écran qui compte le plus.
- **Pourquoi ce code-là, et dans cet ordre** : la messagerie d'un opérateur n'est pas un service à part, c'est un renvoi conditionnel posé sur la ligne à la livraison, souvent avec un délai plus court que celui du client. Les deux coexistent et le plus court gagne, donc l'agent ne sonne jamais. `##002#` est l'effacement GLOBAL (3GPP TS 22.030) et non un code par type : un `##61#` laisserait justement en place celui que le client ne voit pas. Composé APRÈS, il effacerait le renvoi qu'on vient de poser, et la page le dit.
- **La vérification est devenue une preuve, elle n'était qu'une signature** : `forwardingVerifiedAt` était faux par construction, à deux endroits. Le cron quotidien `forwarding-verification` horodatait la colonne après avoir seulement JOURNALISÉ un numéro, sans passer le moindre appel ; la liste d'installation lisait ce champ, donc l'étape « renvoi » naissait verte pour tout le monde — le même défaut que l'étape du numéro de transfert, à un autre endroit. `forwarding-proof.service.ts` horodate maintenant sur la seule chose qui prouve un renvoi : un appel entrant portant un en-tête de diversion qui nomme la ligne du client. Le cron ne fait plus que NOMMER les clients sans preuve depuis sept jours. Tests : `__tests__/forwarding-proof.test.ts`, 8 cas, dont un qui relit le source du cron pour lui interdire de réécrire la colonne.
- **Ce qui manque pour `DÉJÀ FAIT`** : l'appel test lui-même. Un opérateur qui n'envoie aucun en-tête de diversion ne produira jamais de preuve, l'appel se déroulant normalement — et lequel en envoie n'est pas vérifiable ici, aucun appel entrant réel n'ayant encore eu lieu. La déclaration du client continue donc de tenir lieu de réponse, en étant lisiblement une déclaration.
...
- **Statut** : `PARTIEL`
- **Preuve** : Les deux questions sont désormais traitées séparément, dans `backend/src/services/voice/inbound-routing.service.ts`. « Quel client a été appelé » : `divertedNumber` lit `Diversion`, `ForwardedFrom`, `X-Original-Called-Number` dans les cinq sacs d'en-têtes possibles. « Qui appelle » : `callerIdentity` compare le numéro PRÉSENTÉ à celui de la ligne qui renvoie (l'en-tête, ou `businessPhone` en second recours), et quand les deux coïncident il cherche l'appelant dans `P-Asserted-Identity`, `Remote-Party-ID`, `X-Original-Caller`. Le seul point d'entrée du numéro d'appelant sur les appels entrants passe par elle (`realtime-orchestrator.service.ts`, `callerNumberOf`). Tests : `__tests__/inbound-routing.test.ts`, 7 cas pour `callerIdentity`.
- **Le choix qui protège, et qui n'est pas évident** : quand aucun en-tête ne porte l'appelant, la fonction rend `null` au lieu du numéro présenté. Un numéro FAUX est ici pire qu'un numéro absent : il est la clé de la mémoire d'appelant et de l'opposition, et un « ne me rappelez jamais » enregistré sous le numéro du commerce mettrait TOUS ses appelants sur liste d'opposition d'un coup. L'appel se déroule normalement, simplement sans mémoire, et un avertissement nomme le défaut — qui se corrige chez l'opérateur du client, pas ici.
- **Ce qui manque pour `DÉJÀ FAIT`** : la mesure. Le critère demande 100 % des appels renvoyés chez au moins trois opérateurs, et aucun appel entrant réel n'a encore été passé : zéro opérateur testé sur trois. Les noms d'en-têtes retenus sont ceux du SIP courant, pas ceux qu'un opérateur belge a réellement envoyés.
...
- **Statut** : `DÉJÀ FAIT`
- **Preuve** : Le produit fait du renvoi et rien que du renvoi : la fiche d'installation est entièrement bâtie dessus (`frontend/src/pages/client/ClientSetupForwarding.tsx`), et aucun portage n'est implémenté nulle part. L'avertissement qui manquait est désormais écrit aux deux endroits que le critère nomme. Procédure commerciale : `docs/playbook-vente.md`, section « Le numéro : renvoi d'abord, portage seulement après preuve », avec la phrase à dire telle quelle. Fiche d'installation : encart « Vous gardez votre numéro », au-dessus du bouton de fin, là où le client décide.
- **Les deux choses qu'un commerçant ne peut pas découvrir seul**, et qui sont la raison d'être de cette ligne : le portage est irréversible en pratique, et le numéro d'un commerce est souvent couplé à son pack internet — le porter peut résilier la ligne, donc la connexion du magasin. Le renvoi, lui, ne touche à rien et se coupe en trente secondes, ce qui est exactement ce qui rend l'essai sans risque.
...
- **Le parcours de bout en bout existe désormais, et il a trouvé un trou** : `__tests__/compliance-disclosure.test.ts` (« le mode sans enregistrement, de bout en bout », 5 cas) vérifie que les trois endroits tombent du même côté — le drapeau de l'assistant, la phrase d'accueil, et la conservation de l'URL. Il fige aussi l'invariant qui protège le plus : couper l'enregistrement ne coupe PAS l'annonce IA, les deux obligations venant de deux textes différents.
- **Le trou, et il n'était pas théorique** : la fin d'appel écrivait `recordingUrl` sans condition. Un assistant périmé chez Vapi, un réglage changé en cours d'appel ou une surcharge d'escouade suffisait donc à conserver un enregistrement chez un client à qui l'on venait de dire qu'il n'était pas enregistré. `finalizeCall` rend maintenant `recordingAllowed`, et le webhook refuse l'URL en la journalisant (`__tests__/webhook-routes.test.ts`, 2 cas). Le doute penche vers « on garde » quand le profil est illisible : jamais vers un enregistrement fabriqué en cachette.
- **Ce qui manque pour `DÉJÀ FAIT`** : l'écran. Le réglage se pose par l'API, pas encore par une case à cocher dans le portail.
...
- **Statut** : `PARTIEL`
- **Preuve** : Cinq des neuf cas ont désormais un test avec une assertion explicite (`__tests__/adversite.test.ts`, 10 cas) : l'interruption injectée à 200, 500 et 1500 ms — les trois instants du critère, et ils ne se ressemblent pas, les deux premiers tombant sous le plancher de l'interruption dure ; la double-parole d'une seconde et demie, opposée à l'acquiescement coupé court ; le silence total, où le relais d'inactivité doit relancer plus d'une fois sans harceler une ligne raccrochée ; le DTMF en cours d'énoncé, armé sur TOUS les appels parce que l'assistant ne se reconstruit pas en cours d'appel ; et le raccrochage brutal alors que l'agent tient encore la parole. S'y ajoutent, en texte et depuis avant : un transcript inintelligible (`fr-bruit-interruption`) et l'injection de prompt (`fr-injection-appelant`).
- **Ce qui reste, et pourquoi ça ne se simule pas** : le SNR paramétrable (20/10/5/0 dB), les accents, et le changement de sujet en plein remplissage de slots. Les deux premiers demandent le harnais audio qui n'existe pas (TST-1, TST-2) ; les simuler en texte donnerait un test vert qui ne prouve rien, ce qui est pire que pas de test. Le troisième demande un tour de modèle, donc un scénario d'éval — écrit à l'aveugle il serait une régression en puissance, la leçon de 6quinquies étant qu'un ajout se relit contre les scénarios EXISTANTS avant d'être cru.
...
- **Statut** : `DÉJÀ FAIT`
- **Preuve** : `backend/src/services/voice/voice-tracing.ts`. Un span racine par appel, un span par tour, un span par étage MESURÉ, un span par appel d'outil. Export OTLP/HTTP, prouvé par un collecteur local dans `__tests__/voice-tracing.test.ts` (12 cas) : les assertions portent sur la charge REÇUE, pas sur nos objets — un test qui espionnerait `startSpan` prouverait qu'on appelle une bibliothèque, pas qu'un collecteur voit quelque chose.
- **Rétroactif, et pourquoi** : les spans ne sont pas ouverts en direct autour du code du tour. Le pipeline est un flux d'événements Vapi sans pile d'appel commune ; envelopper le tour demanderait de porter un contexte OTel à travers un webhook, un flux SSE et un pool de promesses, pour instrumenter du code qui n'attend rien. Les marques restent celles de `latency-tracker`, et le span est CRÉÉ à la fermeture du tour avec ses horodatages explicites. Un test vérifie qu'un span créé après coup porte bien la durée mesurée, pas l'heure de sa création.
- **Une seule règle de validité** : `push()` écrit l'échantillon ET la fenêtre du span. Un étage écarté par la borne de plausibilité n'apparaît donc ni dans les percentiles ni dans la trace. Deux règles séparées finiraient par faire dire à la trace autre chose qu'aux agrégats, sur le même appel.
- **Deux registres, jamais mélangés** : `gen_ai.*` (la convention officielle) pour ce qu'elle couvre — le modèle et les outils ; `qwillio.voice.*` pour ce qu'elle ne couvre pas — STT, TTS, TTFA, barge-in. Le préfixe est ce qui rendra la bascule mécanique le jour où une convention voix sortira, au lieu d'avoir à démêler des attributs qui se seraient fait passer pour standard. Un test interdit tout `gen_ai.*` hors de l'étage LLM.
- **Ce qui n'entre JAMAIS dans un span** : aucun transcript, aucun énoncé, aucun argument d'outil, aucun numéro d'appelant. Une trace part chez un tiers qui n'est pas dans le périmètre de traitement du client ; y verser une conversation serait une divulgation, pas une observabilité. Un test liste les mots interdits dans les clés d'attributs et tombera le jour où quelqu'un ajoutera « juste le dernier énoncé, pour déboguer ».
- **Éteint par défaut** : sans `OTEL_EXPORTER_OTLP_ENDPOINT`, le SDK n'est même pas chargé. Un défaut `localhost:4318` ferait accumuler un lot non livrable et réessayer, en silence, sur un service qui redéploie plusieurs fois par jour. `flush()` et `shutdown()` sont séparés de propos délibéré : `provider.shutdown()` éteint l'exportateur pour de bon, et un « flush » qui rendrait l'instrumentation muette pour le reste de la vie du process ne se remarque qu'en cherchant les traces qui manquent.
...
- **L'histogramme est à l'écran (10/09)** : `GET /my-dashboard/calls/abandonment` et `AbandonByTurn`, posé en tête de la page Appels — un abandon au premier tour est une panne d'installation, et la lire dans une liste d'appels un par un ne la fait jamais apparaître. Le découpage est une fonction PURE partagée avec le rapport (`abandonHistogram`) : deux copies d'un découpage finissent toujours par ne plus raconter la même chose. Recalculé à la demande sur une fenêtre bornée plutôt que rangé quelque part, sinon l'écran décrirait la semaine où le cron est passé.
- **Deux différences avec le rapport, et elles comptent** : le rapport ne publie qu'au-delà du seuil, ce qui est juste pour une alerte et faux pour un écran — un client qui vient regarder son chiffre doit le trouver, y compris quand il est bon. Et sous le plancher d'appels, l'écran montre la forme sans conclure, au lieu d'afficher 100 % sur un seul appel raté. La route n'est PAS derrière `advancedAnalytics` : c'est le petit forfait qui en a le plus besoin.
- **Ce qui manque** : il n'a encore rien à décrire, aucun appel entrant réel n'ayant eu lieu. L'encart s'efface tout seul tant que c'est le cas, plutôt que d'afficher un zéro qui ressemble à une réponse.
...
|---|---|---|---|---|
| 2026-09-10 | LAT-7 | preuve complétée, reste `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | L'accueil pré-enregistré atteint enfin un appel entrant. Variante figée à 0, arbitrage tranché : la même phrase pour tous contre la latence sur le mot qui décide de l'impression. L'ordre porte tout le risque — la synchronisation efface les lignes avant de les refaire, donc épingler l'URL sans attendre ouvrait une fenêtre où chaque appel s'ouvrait sur du silence. |
...
| 2026-09-10 | TST-7 | `ABSENT` → `DÉJÀ FAIT` | `claude/optimisations-audit-vocal-68tgg2` | Les percentiles répondent à « la flotte a-t-elle ralenti ? » et à aucune des questions qu'on se pose quand un client rappelle : un percentile n'a ni tour, ni appel, ni ordre, donc il ne se remonte pas jusqu'à un incident. Les spans sont créés à la fermeture du tour avec les horodatages du tracker, jamais autour du code : le chemin chaud ne gagne pas une allocation et la mesure ne bouge pas d'une milliseconde. Éteint tant qu'aucun collecteur n'est configuré. |
...
| 2026-09-10 | REL-7 | preuve complétée, reste `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | L'entonnoir de transfert passe de la route de santé, où il décrit la flotte, à l'écran du client, où il répond à « pourquoi l'agent ne m'a pas passé l'appel ». Deux refus assumés : pas d'étape « ça a sonné », que rien n'enregistre, et un échec sans cause reste compté sous un libellé qui le dit. Reste le vrai code SIP, à récupérer chez l'opérateur. |
| 2026-09-10 | TST-9 | preuve complétée, reste `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | L'histogramme d'abandon passe du rapport hebdomadaire à l'écran, en tête de la page Appels. Découpage partagé avec le rapport par une fonction pure, recalculé à la demande, et deux règles propres à l'écran : on publie même sous le seuil (un bon chiffre se regarde aussi) mais on ne conclut pas sous le plancher d'appels. Il reste sans rien à décrire tant qu'aucun appel entrant réel n'a eu lieu, et s'efface plutôt que d'afficher un zéro. |
| 2026-09-10 | TUR-6 | preuve complétée, reste `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | Les deux chemins partageaient un seul seuil de voix alors qu'ils n'ont pas les mêmes défenses : en classique le bruit est trié deux fois, en parole-à-parole `voiceSeconds` porte tout seul. Le curseur est séparé, à défaut inchangé — le monter change la charge envoyée à Vapi, et un hors-bornes emporte la flotte entière. |
| 2026-09-10 | TST-6 | preuve complétée, reste `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | Cinq des neuf modes d'échec adverses ont maintenant une assertion explicite : interruption à 200/500/1500 ms, double-parole d'1,5 s, silence total, DTMF en cours d'énoncé, raccrochage brutal. Les axes bruit et accent restent dehors, faute de harnais audio : les simuler en texte donnerait un vert qui ne prouve rien. |
| 2026-09-10 | TUR-13 | `ABSENT` → `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | Le côté cher de l'arbitrage n'était compté nulle part : deux compteurs disaient que l'appelant coupe l'agent, aucun ne disait que l'agent coupe l'appelant. `falseCuts` le relève sur deux conditions conjointes (reprise sous 1,2 s ET tour précédent inachevé), et le taux est publié à chaque analyse avec la cible de 5 %. Ce que ça règle est l'endpointing : le seuil de confiance du transcripteur n'est pas exposé par Vapi. |
| 2026-09-10 | REL-12 | `PARTIEL` → `DÉJÀ FAIT` | `claude/optimisations-audit-vocal-68tgg2` | L'avertissement était le seul morceau manquant, et il l'était aux deux endroits exigés. Écrit dans le playbook de vente et dans la fiche d'installation : le portage est irréversible en pratique, et sur un fixe le numéro est souvent couplé au pack internet, donc le porter peut couper la connexion du commerce. Le renvoi, lui, ne touche à rien. |
| 2026-09-10 | REL-10 | `ABSENT` → `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | L'étape qui manquait passe en tête de la fiche d'installation, et la case « vérifié » cesse de mentir. La messagerie de l'opérateur est un renvoi conditionnel posé sur la ligne, avec un délai plus court : sans `##002#` d'abord, elle capte l'appel et l'agent ne sonne jamais. Et `forwardingVerifiedAt` s'écrivait par cron sans qu'aucun appel ne soit passé, donc l'étape naissait verte pour tout le monde ; elle s'écrit désormais sur un appel réellement renvoyé, ou pas du tout. |
| 2026-09-10 | TUR-9 | `ABSENT` → `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | L'historique du chemin custom-LLM ne porte plus que la part JOUÉE du dernier énoncé. Le pipeline génère plus vite que le temps réel : à l'interruption, le modèle avait écrit trois créneaux et l'appelant en avait entendu un. La durée vient du barge-in, jamais d'un horodatage au mot — Vapi n'en rend aucun — donc c'est une estimation par débit de parole, franche dans le sens qui coûte le moins : au pire l'agent se répète, jamais il ne s'appuie sur ce que personne n'a entendu. |
| 2026-09-09 | REL-2 | `ABSENT` → `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | L'intention est couverte par `voice.fallbackPlan` (deux voix de secours, identifiants réels par défaut), et la première phrase survit déjà à un TTS mort puisque l'accueil pré-rendu est servi en octets. La banque de phrases EN COURS d'appel est hors de portée: Vapi n'accepte de l'audio pré-enregistré qu'en `firstMessage`, et le flux média lui appartient. Écrit pour que personne ne construise l'impossible. |
| 2026-09-09 | REL-11 | preuve complétée, reste `PARTIEL` | `claude/optimisations-audit-vocal-68tgg2` | `callerIdentity` sépare « qui appelle » de « quel client a été appelé ». Quand le numéro présenté est celui de la ligne qui renvoie, l'appelant est cherché dans `P-Asserted-Identity` et consorts, et à défaut on rend `null`: un numéro faux porterait la mémoire d'appelant et l'OPPOSITION, qui vaudrait alors pour tous les appelants du client. Reste la mesure, qui demande des appels renvoyés réels chez trois opérateurs. |
| 2026-09-09 | BEL-6 | `ABSENT` → `DÉJÀ FAIT` | `claude/optimisations-audit-vocal-68tgg2` | Les dix-neuf bruxelloises et la périphérie, 45 paires dont les noms diffèrent, la commune dictée réécrite dans la langue du client, et un champ `address` dans les outils. Deux pièges réintroduits une fois pour vérifier que le test tombe : la rue traduite à la place de la commune, et une paire dont les deux noms sont identiques. |
| 2026-09-09 | BEL-4, REL-8, BEL-5 | preuves confirmées par l'API vivante | `claude/optimisations-audit-vocal-68tgg2` | `npm run voice:validate` accepte les six variantes sur Render. Le premier passage en avait refusé six sur six : `keypadInputPlan.delimiters` veut un TABLEAU, contre ce que dit la documentation, et le nom d'un assistant est plafonné à 40 caractères — ce second point cassait la création d'assistant pour toute entreprise au nom un peu long, en production. Vapi rend ses erreurs par lot : deux champs fautifs en masquaient quatre autres. |
