 ...
 * data is all we want: extracting it at build time means the exact artwork with
 * none of the runtime — no player library, no JSON parsed in the browser, and
 * the paths drop straight into the animation already in place.
 */
...
const frames = path.ks.k.map(kf => toPath(kf.s ? kf.s[0] : kf.s));
