                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                        call.isLead ? 'bg-amber-400/10' : 'bg-[#7349fe]/10'
                      }`}
                      aria-hidden="true"
                    >
                      {call.isLead
                        ? <Users size={14} className="text-amber-400" />
                        : <Phone size={14} className="text-[#7349fe]" />
                      }
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {call.isLead
                        ...
                        : <Phone size={16} className="text-[#7349fe]" />
                      }
                    </div>
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      selectedCall.isLead ? 'bg-amber-400/10' : 'bg-[#7349fe]/10'
                    }`}
                    aria-hidden="true"
                  >
                    {selectedCall.isLead
                      ? <Users size={22} className="text-amber-400" />
                      : <Phone size={22} className="text-[#7349fe]" />
                    }
                  </div>
                  <div>
                ...
                {selectedCall.leadScore != null && (
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <div
                          className="h-full bg-[#7349fe] rounded-full"
                          style={{ width: `${selectedCall.leadScore * 10}%` }}
                        />
                      </div>
                      <span className="text-sm font-bold text-[#7349fe]">{selectedCall.leadScore}/10</span>
                    </div>
                  </div>
                )}
                ...
                {selectedCall.recordingUrl && (
                  <div>
                    ...
                    <div className="rounded-xl bg-white/[0.04] border border-white/[0.07] p-4">
                      {/* Un seul bouton, la pilule blanche du site, qui
                          s'allonge au clic pour révéler la timeline et le
                          temps (demande utilisateur du 13/09 : « pas deux
                          boutons et un lecteur »). Remonté par `key` à chaque
                          appel, pour que l'enregistrement d'un appel ne
                          reste jamais affiché sous un autre. */}
                      <RecordingPlayer key={selectedCall.id} callId={selectedCall.id} />
                    </div>
                  </div>
                )}
