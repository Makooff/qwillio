    ...
    <div>
      ...
      {selected.size > 0 && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          className="flex items-center gap-3 mb-4 px-4 py-3 bg-[#7349fe]/5 border border-[#7349fe]/20 rounded-xl">
          <span className="text-sm font-medium text-[#7349fe]">{selected.size} selected</span>
      ...
      <div className="rounded-2xl border border-[#d2d2d7]/60 bg-white overflow-hidden mb-4">
        ...
        <div className="grid grid-cols-[2rem_2fr_1.5fr_1.2fr_1fr_1.2fr_1.5fr_1fr_2rem] items-center gap-3 px-5 py-3 border-b border-[#f5f5f7] bg-[#fafafa]">
          <button type="button" onClick={toggleAll} className="text-[#86868b] hover:text-[#7349fe]">
            {selected.size === filtered.length && filtered.length > 0
              ? <CheckSquare size={15} className="text-[#7349fe]" />
              : <Square size={15} />
            }
          </button>
          ...
          filtered.map((c, idx) => {
            ...
            return (
              <motion.div key={c.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }}
                ...
                <button type="button" onClick={() => toggleSelect(c.id)} className="text-[#86868b] hover:text-[#7349fe]">
                  {selected.has(c.id)
                    ? <CheckSquare size={15} className="text-[#7349fe]" />
                    : <Square size={15} />
                  }
                ...
                <button type="button" className="opacity-0 group-hover:opacity-100 transition-opacity text-[#86868b] hover:text-[#7349fe]">
          ...
          {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
            <button
              ...
              className={`w-8 h-8 rounded-lg text-xs font-medium border transition-colors ${
                page === p ? 'bg-[#7349fe] text-white border-[#7349fe]' : 'border-[#d2d2d7]/60 bg-white hover:bg-[#f5f5f7] text-[#1d1d1f]'
              }`}
            >{p}</button>
          ))}
          <button
