    ...
    <div>
      ...
      >
        <div className="flex-[2] p-6">
          <p className="text-[10px] font-semibold text-[#86868b] uppercase tracking-widest mb-2">Total Products</p>
          <p className="text-4xl font-bold tracking-tight">{products.length}</p>
          <p className="text-[10px] text-[#86868b]/50 mt-2">tracked in system</p>
        </div>
        <div className="flex-1 p-6 bg-red-50/50">
          <p className="text-[10px] font-semibold text-red-400 uppercase tracking-widest mb-2">Low Stock</p>
          <p className="text-2xl font-bold text-red-600">{lowStockProducts.length}</p>
          <p className="text-[10px] text-red-400/60 mt-2">need reorder</p>
        </div>
        <div className="flex-1 p-6">
          <p className="text-[10px] font-semibold text-[#86868b] uppercase tracking-widest mb-2">Stock Value</p>
          <p className="text-2xl font-bold">${totalStockCost.toFixed(0)}</p>
          <p className="text-[10px] text-[#86868b]/50 mt-2">on hand</p>
        </div>
        <div className="flex-1 p-6">
          <p className="text-[10px] font-semibold text-[#86868b] uppercase tracking-widest mb-2">Monthly Usage</p>
          <p className="text-2xl font-bold">${monthlyUsageCost.toFixed(0)}</p>
          <p className="text-[10px] text-[#86868b]/50 mt-2">this month</p>
        </div>
      </motion.div>
            ...
            {lowStockProducts.map(p => (
              <div key={p.id} className="flex items-center justify-between bg-white rounded-xl px-4 py-3 border border-red-100">
                ...
                {p.autoOrder ? (
                  ...
                  <button className="text-xs font-medium text-[#7349fe] hover:underline">Order now</button>
                )}
              </div>
            ))}
          </div>
                  ...
                  >
                    ...
                    <td className="py-3 text-center">
                      <button onClick={() => toggleAutoOrder(p.id)}>
                        {p.autoOrder
                          ? <ToggleRight size={20} className="text-[#7349fe] mx-auto" />
                    ...
                    <td className="py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          ...
                          className="text-[#86868b] hover:text-[#7349fe] transition-colors"
        ...
        <div className="rounded-2xl border border-[#d2d2d7]/60 bg-white p-6">
          ...
          <div className="space-y-3">
              ...
              return (
                <div key={p.id}>
                  ...
                  <div className="h-1.5 bg-[#f5f5f7] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-[#7349fe] rounded-full transition-[width] duration-500 ease-out"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
            ...
            <div className="pt-3 border-t border-[#d2d2d7]/40 flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-[#7349fe]">${totalStockCost.toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-sm border border-[#d2d2d7]/60 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#7349fe]/30"
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setNewProduct(p => ({ ...p, autoOrder: !p.autoOrder }))}>
                    {newProduct.autoOrder
                      ? <ToggleRight size={24} className="text-[#7349fe]" />
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  className="px-5 py-2.5 text-sm font-medium bg-[#f5f5f7] text-[#1d1d1f] rounded-xl hover:bg-[#e8e8ed] border border-[#d2d2d7]/60 transition-colors"
                >
                  Cancel
                </button>
              </div>
