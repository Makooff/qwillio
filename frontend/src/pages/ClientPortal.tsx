...
// Portal data shape is intentionally loose — the backend returns heterogeneous nested
// objects per tab that the legacy UI accesses dynamically. Tightening here would cascade
// dozens of casts without runtime gain. Public portal page only.
...
export default function ClientPortal() {
  ...
  const [data, setData] = useState<PortalData>({ overview: null, calls: null, bookings: null, leads: null, analytics: null });
  ...
  const fetchData = async () => {
    if (!clientId || !token) { setError(t('portal.error.link')); setLoading(false); return; }
    try {
      setLoading(true);
      const [overview, calls, bookings, leads, analytics] = await Promise.all([
        api.get(`/client-portal/${clientId}/overview`),
        api.get(`/client-portal/${clientId}/calls?page=1&limit=20`),
        api.get(`/client-portal/${clientId}/bookings`),
        api.get(`/client-portal/${clientId}/leads?page=1&limit=20`),
        api.get(`/client-portal/${clientId}/analytics?days=30`),
      ]);
      setData({
  ...
  useEffect(() => { fetchData(); }, [clientId, token]);
        ...
        <div className="max-w-[1120px] mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-medium">{ov.businessName || t('portal.business.default')}</p>
