...
const OC: Record<string,string> = { interested:'#818cf8', not_interested:'rgba(255,80,80,0.7)', no_answer:'rgba(255,255,255,0.25)', callback:'#a5b4fc', converted:'#6366F1' };
...
const Calls: React.FC = () => {
  ...
  if(loading) return <div style={{minHeight:'100vh',background:'#0a0a0a',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{width:28,height:28,borderRadius:'50%',border:'2px solid #6366F1',borderTopColor:'transparent',animation:'spin .8s linear infinite'}}/><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>;
  return (
        ...
        <div style={{background:'#161616',borderRadius:16,border:'1px solid rgba(255,255,255,0.06)',overflow:'hidden'}}>
          ...
          {items.map((c,i)=>(
            <div key={c.id} style={{display:'flex',alignItems:'center',gap:14,padding:'13px 16px',borderBottom:i<items.length-1?'1px solid rgba(255,255,255,0.05)':'none'}}>
              <div style={{width:42,height:42,borderRadius:'50%',background:OC[c.outcome]+'18'||'rgba(255,255,255,0.08)',border:`1.5px solid ${OC[c.outcome]||'rgba(255,255,255,0.1)'}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:18}}>ðŸ“ž</div>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:12,color:'rgba(255,255,255,0.3)',marginTop:2}}>{c.duration?`${Math.floor(c.duration/60)}m${c.duration%60}s`:'â€”'}</div>
              </div>
