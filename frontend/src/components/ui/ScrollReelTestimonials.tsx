...
/* ----------------------------------------------------------------
 * ScrollReelTestimonials — counter-rotating reel + per-character rise.
 * Adapted for the Qwillio marketing register (Vite + plain Tailwind):
 * shadcn tokens mapped to the cream/ink palette, brand indigo sheen.
 * ---------------------------------------------------------------- */
...
const AUTHOR_CLASSES = 'm-0 text-sm font-medium leading-[1.3] text-[#86868b]';
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-[#1d1d1f]/10 bg-gradient-to-b from-[#ececf0] to-white blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(255,255,255,1)]"
...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    <div
      className="relative shrink-0 overflow-hidden rounded-xl bg-[#ececf0]"
...
/* Per-character split; spaces stay as text nodes so wrapping is natural. */
function Chars({
  text,
  startIndex,
  staggerMs,
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          <svg
            className="block h-12 w-12 text-[#86868b]/40"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
